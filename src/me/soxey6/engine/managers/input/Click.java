...
/**
 * A very basic class used for registering clicks and their locations. Please us
 * this instead of Vector2F
 * 
 * @author spanish
 */
public class Click {
	...
	public Click(int x, int y, int mouseButton) {
		try {
			this.setX((float) x
					/ Settings.getSettings().getSetting("RESOLUTION_X")
							.getValueInt());
			this.setY((float) y
					/ Settings.getSettings().getSetting("RESOLUTION_Y")
							.getValueInt());
		} catch (Throwable e) {
			e.printStackTrace();
		}
